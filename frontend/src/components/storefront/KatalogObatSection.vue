      ...
      <div v-else class="obat-grid">
        ...
        >
          ...
          <div class="obat-actions">
            <button
              class="btn-cart"
              :disabled="obat.stok === 0 || loadingCart[obat.id]"
              @click="addToCart(obat)"
            >
...
export default {
  ...
  computed: {
    sortedObat() {
      ...
      if (this.filterJenisId) {
        list = list.filter(o => o.idjenis === this.filterJenisId)
      }
  ...
  methods: {
    ...
    async addToCart(obat) {
      ...
      try {
        await api.post('/keranjang', { id_obat: obat.id, jumlah_order: 1 })
...
<style scoped>
.section { padding: 5rem 0; background: #f8fafb; }
...
.katalog-meta { display: flex; align-items: center; gap: 1rem; }
.total-label { font-size: 0.8rem; color: #6b7280; }
.sort-select {
  padding: 0.4rem 0.75rem; border: 1.5px solid #e5e7eb; border-radius: 0.5rem;
  font-size: 0.8rem; font-family: inherit; color: #374151; outline: none;
  background: #fff; cursor: pointer;
}
.sort-select:focus { border-color: #059669; }
...
/* Image */
.obat-img-wrap { position: relative; aspect-ratio: 1; overflow: hidden; background: #f0fdf4; }
.obat-img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.3s; }
.obat-card:hover .obat-img { transform: scale(1.05); }
.obat-img-placeholder {
  width: 100%; height: 100%; display: flex; align-items: center; justify-content: center;
}
.obat-img-placeholder .material-symbols-outlined { font-size: 3rem; color: #a7f3d0; }
...
/* Info */
.obat-info { padding: 0.875rem; flex: 1; display: flex; flex-direction: column; gap: 0.35rem; }
.obat-name { font-size: 0.875rem; font-weight: 700; color: #171c1f; line-height: 1.3; }
.obat-desc { font-size: 0.72rem; color: #9ca3af; line-height: 1.5; flex: 1; }
.obat-price { font-family: 'Manrope', sans-serif; font-size: 1rem; font-weight: 800; color: #059669; }

/* Actions */
.obat-actions { padding: 0 0.875rem 0.875rem; }
.btn-cart {
  width: 100%; display: flex; align-items: center; justify-content: center; gap: 0.4rem;
  padding: 0.55rem; border-radius: 0.625rem; border: none;
  background: #ecfdf5; color: #065f46; font-size: 0.8rem; font-weight: 700;
  cursor: pointer; transition: all 0.15s; font-family: inherit;
}
.btn-cart:hover:not(:disabled) { background: #059669; color: #fff; }
.btn-cart:disabled { opacity: 0.5; cursor: not-allowed; }
.btn-cart .material-symbols-outlined { font-size: 1rem; }
...
.btn-load-more:hover { border-color: #059669; color: #059669; background: #ecfdf5; }

/* Error */
.error-state { display: flex; flex-direction: column; align-items: center; gap: 0.75rem; padding: 3rem; color: #9ca3af; }
.error-state .material-symbols-outlined { font-size: 2.5rem; color: #fca5a5; }
.btn-retry { padding: 0.5rem 1.25rem; border: none; border-radius: 0.5rem; background: #059669; color: #fff; font-weight: 700; cursor: pointer; font-family: inherit; }
