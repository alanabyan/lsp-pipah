      ...
      <div class="checkout-hero">
        <div class="container">
          ...
          <div>
            ...
            <p class="checkout-sub">Review your health essentials and finalize your clinical delivery.</p>
          </div>
        </div>
      </div>
              ...
              <div v-else class="cart-items">
                <div v-for="item in cartItems" :key="item.id" class="cart-item">
                  ...
                  <div class="item-info">
                    ...
                    <p class="item-meta">{{ item.obat?.deskripsi_obat | truncate }}</p>
                    <div class="item-qty">
                  ...
                  <div class="item-right">
                    ...
                    <button class="remove-btn" @click="removeItem(item.id)" title="Hapus">
            ...
            <div class="card">
              ...
              <div class="delivery-grid">
                ...
                <div class="form-group">
                  ...
                  <div v-if="loadingShipping" class="shipping-skeleton"></div>
                  <div v-else class="shipping-options">
                    <label
                      v-for="ship in shippingMethods"
                      :key="ship.id"
                    ...
                    >
                      ...
                      <span class="shipping-price">
                        {{ ship.jenis_kirim === 'standar' ? 'Free' : formatRp(12000) }}
                      </span>
                    </label>
                  </div>
                </div>
          ...
          <div class="checkout-right">
            <div class="summary-card">
              ...
              <div class="payment-section">
                ...
                <div v-if="loadingPayment" class="payment-skeleton"></div>
                <div v-else class="payment-options">
                  ...
                  >
                    ...
                    <span class="material-symbols-outlined pay-icon">
                    ...
                    <div>
                      ...
                      <p class="pay-sub">{{ pm.tempat_bayar }}</p>
                    </div>
                  </label>
                </div>
              </div>
              ...
              >
                ...
                {{ loadingOrder ? 'Memproses...' : 'Confirm Order →' }}
              </button>

              <!-- Guarantee -->
              <div class="guarantee-note">
                <span class="material-symbols-outlined">verified_user</span>
                <p>Your order is protected by our <strong>Encounter Guarantee</strong>. All medications are from BPOM-certified sources.</p>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    ...
    this.deliveryForm.alamat = pelanggan.alamat1 || ''
    this.fetchCart()
    this.fetchShipping()
  ...
  methods: {
    ...
    async updateQty(item, newQty) {
      if (newQty < 1) return
      try {
        await api.put(`/keranjang/${item.id}`, { jumlah_order: newQty })
        await this.fetchCart()
      } catch (err) {
        this.showToast(err.response?.data?.message || 'Gagal update jumlah', 'error')
      }
    },
...
<style scoped>
.checkout-page { font-family: 'Plus Jakarta Sans', sans-serif; background: #0a0a12; min-height: 100vh; }

.checkout-wrap { padding-top: 4.5rem; }
...
.checkout-hero .container { display: flex; align-items: center; gap: 1rem; }
.checkout-hero-icon {
  width: 3rem; height: 3rem; border-radius: 0.75rem;
  background: rgba(74,222,128,0.12); display: flex; align-items: center; justify-content: center;
}
.checkout-hero-icon .material-symbols-outlined { color: #4ade80; font-size: 1.5rem; }
.checkout-title { font-family: 'Manrope', sans-serif; font-size: 1.75rem; font-weight: 900; color: #fff; letter-spacing: -0.03em; }
.checkout-sub   { font-size: 0.82rem; color: rgba(255,255,255,0.4); margin-top: 2px; }

.container { max-width: 1100px; margin: 0 auto; padding: 0 1.5rem; }

/* Grid */
.checkout-grid { display: grid; grid-template-columns: 1fr 22rem; gap: 1.5rem; padding: 2rem 0 4rem; }
...
.card-icon  { font-size: 1.1rem; color: #4ade80; }
.card-title { font-family: 'Manrope', sans-serif; font-size: 1rem; font-weight: 700; color: #fff; }
.item-count { font-size: 0.75rem; color: rgba(255,255,255,0.35); background: rgba(255,255,255,0.06); padding: 0.2rem 0.6rem; border-radius: 9999px; }
...
.item-img .material-symbols-outlined { color: rgba(255,255,255,0.2); font-size: 1.5rem; }
.item-info { flex: 1; }
.item-name { font-size: 0.875rem; font-weight: 700; color: #fff; }
.item-meta { font-size: 0.7rem; color: rgba(255,255,255,0.35); margin-top: 2px; }
.item-qty  { display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem; }
.qty-btn {
...
.qty-btn:hover { background: rgba(74,222,128,0.15); border-color: #4ade80; color: #4ade80; }
.qty-val { font-size: 0.82rem; font-weight: 700; color: #fff; min-width: 1.5rem; text-align: center; }
.item-right { display: flex; flex-direction: column; align-items: flex-end; gap: 0.5rem; }
.item-price { font-family: 'Manrope', sans-serif; font-size: 0.95rem; font-weight: 800; color: #fff; }
.remove-btn {
...
/* Delivery */
.delivery-grid  { display: flex; flex-direction: column; gap: 1.25rem; }
.form-group     { display: flex; flex-direction: column; gap: 0.4rem; }
.form-label     { font-size: 0.75rem; font-weight: 700; color: rgba(255,255,255,0.5); text-transform: uppercase; letter-spacing: 0.05em; }
.form-textarea {
  padding: 0.75rem; background: rgba(255,255,255,0.04);
  border: 1.5px solid rgba(255,255,255,0.08); border-radius: 0.625rem;
  color: #fff; font-size: 0.875rem; outline: none; resize: none;
  font-family: inherit; transition: border 0.15s;
}
.form-textarea:focus { border-color: #4ade80; background: rgba(74,222,128,0.03); }
...
.shipping-option--active { border-color: #4ade80; background: rgba(74,222,128,0.06); }
.radio-input { display: none; }
.shipping-info { flex: 1; }
.shipping-name { font-size: 0.875rem; font-weight: 700; color: #fff; }
.shipping-type { font-size: 0.7rem; color: rgba(255,255,255,0.4); text-transform: capitalize; }
.shipping-price { font-size: 0.82rem; font-weight: 700; color: #4ade80; }
.free-text { color: #4ade80; }
...
.summary-title { font-family: 'Manrope', sans-serif; font-size: 1rem; font-weight: 700; color: #fff; }
.summary-rows  { display: flex; flex-direction: column; gap: 0.6rem; }
.summary-row   { display: flex; justify-content: space-between; font-size: 0.82rem; color: rgba(255,255,255,0.5); }
.summary-total {
  display: flex; justify-content: space-between; align-items: center;
  padding-top: 1rem; border-top: 1px solid rgba(255,255,255,0.07);
  font-size: 0.875rem; font-weight: 700; color: #fff;
}
.total-val { font-family: 'Manrope', sans-serif; font-size: 1.4rem; font-weight: 900; color: #4ade80; }

/* Payment */
.payment-label { font-size: 0.72rem; font-weight: 700; color: rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 0.06em; }
.payment-options { display: flex; flex-direction: column; gap: 0.5rem; margin-top: 0.5rem; }
.payment-option {
...
.payment-option--active { border-color: #4ade80; background: rgba(74,222,128,0.05); }
.pay-icon { font-size: 1.1rem; color: #4ade80; }
.pay-name { font-size: 0.8rem; font-weight: 700; color: #fff; }
.pay-sub  { font-size: 0.68rem; color: rgba(255,255,255,0.35); }

/* Confirm btn */
.btn-confirm {
  width: 100%; display: flex; align-items: center; justify-content: center; gap: 0.5rem;
  padding: 0.875rem; border: none; border-radius: 0.75rem;
  background: #4ade80; color: #052e16; font-size: 0.9rem; font-weight: 800;
  cursor: pointer; transition: all 0.2s; font-family: inherit;
  box-shadow: 0 4px 20px rgba(74,222,128,0.3);
}
.btn-confirm:hover:not(:disabled) { background: #22c55e; transform: translateY(-2px); box-shadow: 0 8px 28px rgba(74,222,128,0.4); }
.btn-confirm:disabled { opacity: 0.4; cursor: not-allowed; }
.btn-confirm .material-symbols-outlined { font-size: 1.1rem; }
...
.guarantee-note .material-symbols-outlined { font-size: 1rem; color: #4ade80; flex-shrink: 0; margin-top: 1px; }
.guarantee-note p { font-size: 0.72rem; color: rgba(255,255,255,0.5); line-height: 1.5; }
.guarantee-note strong { color: rgba(255,255,255,0.7); }

/* Skeletons */
.cart-skeleton, .shipping-skeleton, .payment-skeleton {
  height: 4rem; border-radius: 0.625rem;
  background: linear-gradient(90deg, rgba(255,255,255,0.04) 25%, rgba(255,255,255,0.08) 50%, rgba(255,255,255,0.04) 75%);
  background-size: 200% 100%; animation: shimmer 1.4s infinite; margin-bottom: 0.75rem;
}
@keyframes shimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }

/* Empty */
.empty-state { display: flex; flex-direction: column; align-items: center; gap: 0.75rem; padding: 2rem; color: rgba(255,255,255,0.25); }
